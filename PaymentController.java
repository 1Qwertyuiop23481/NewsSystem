...
public class PaymentController {
    ...
    public PaymentController(PaymentService paymentService, UserService userService, BookingService bookingService,
        ...
        try {
            ...
            if (!booking.getUser().getId().equals(user.getId())) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body("Error: You are not authorized to create a payment for this booking.");
            }
    ...
    public ResponseEntity<?> makePayment(HttpServletRequest request,
        ...
        try {
            ...
            if (!booking.getUser().getId().equals(user.getId())) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body("Error: You are not authorized to make a payment for this booking.");
            }
            ...
            if (payment.getStatus() == PaymentStatus.SUCCESSFUL) {
                return ResponseEntity.ok(Map.of(
                    "status", "success",
                    "message", "Payment was already processed successfully. Your booking is confirmed."
                ));
            }

            // Process the payment
            PaymentDTO paymentDTO = paymentService.makePayment(paymentId, paymentMethod, amount, stripePaymentMethodId);
            return ResponseEntity.ok(Map.of(
                "status", "success",
            ...
            // Check if it's an "already succeeded" error
            if (e.getMessage() != null && e.getMessage().contains("already succeeded")) {
                return ResponseEntity.ok(Map.of(
                    "status", "success",
                    "message", "Payment already processed successfully. Your booking is confirmed."
                ));
            }
            ...
            if (errorMessage != null && errorMessage.contains("already succeeded")) {
                return ResponseEntity.ok(Map.of(
                    "status", "success",
                    "message", "Payment already processed successfully. Your booking is confirmed."
                ));
    ...
    public ResponseEntity<?> getPaymentByBookingId(
        ...
        try {
            ...
            Optional<Payment> existingPayment = paymentRepository.findByBookingIdAndStatus(bookingId,
    ...
    public ResponseEntity<?> getPaymentStatus(
        ...
        try {
            ...
            if (!booking.getUser().getId().equals(user.getId())) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body("Error: You are not authorized to access this payment.");
            }
